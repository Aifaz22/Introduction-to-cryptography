...
def hash_bytes( input ):
	...
	"""
	result = hashes.Hash(hashes.SHA224())
	result.update(input)
...
def find_collision( length ):
	...
	while time.time()<=s+120:
		i = os.urandom(2 ** length)
		hashed=hash_bytes(i)
		trimmed_hash = hashed[:length]
		if trimmed_hash in hashmap:
			return (i, hashmap[trimmed_hash])
		hashmap[trimmed_hash]=i
	return None
